...
def group_by_centroid(restaurants, centroids):
    ...
    for r in restaurants:
        ...
        buckets[tuple(c)].append(r)
    # return clusters in centroid order
...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    "*** YOUR CODE HERE ***"
    xs = [restaurant_location(r)[0] for r in cluster]
    ys = [restaurant_location(r)[1] for r in cluster]
...
def k_means(restaurants, k, max_updates=100):
    ...
    while previous_centroids != centroids and n < max_updates:
        ...
        # recompute centroids from clusters
...
def find_predictor(user, restaurants, feature_fn):
    ...
    b = 0 if Sxx == 0 else Sxy / Sxx
    a = y_bar - b * x_bar
    r_squared = 0 if Sxx == 0 or Syy == 0 else (Sxy ** 2) / (Sxx * Syy)
    # END Question 7
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    "*** YOUR CODE HERE ***"
    best_r2, best_pred = -1, None
    for f in feature_fns:
        pred, r2 = find_predictor(user, reviewed, f)
        if r2 > best_r2:
            best_r2, best_pred = r2, pred
    return best_pred
    # END Question 8
